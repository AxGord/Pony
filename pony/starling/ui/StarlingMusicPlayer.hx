...
using pony.starling.utils.StarlingUtils;
using pony.Tools;
/**
 * StarlingMusicPlayer
 * @author AxGord
...
class StarlingMusicPlayer extends StarlingSongPlayer {
	...
	public function loadPlaylist(pl:Array<SongInfo>):Void {
		...
		for (e in pl) {
			...
			var t:TextField = new TextField(Std.int(_tTitle.width), Std.int(_tTitle.height), SongPlayerCore.formatSong(e), _tTitle.fontName, _tTitle.fontSize, _tTitle.color, _tTitle.bold);
			...
			var t:TextField = new TextField(Std.int(_tTime.width), Std.int(_tTime.height), e.length, _tTime.fontName, _tTime.fontSize, _tTime.color, _tTime.bold);
