...
@:nullSafety(Strict) final class Bmfont extends NModule<BmfontConfig> {
	...
	private function run(cfg: BmfontConfig): Void {
		...
		for (font in cfg.font)
			packFont(
				cfg.from + font.file, font.size, font.face, cfg.type, font.charset, font.output, cfg.format, font.lineHeight, cfg.distance,
				cfg.padding
			);
	}

	private function packFont(
		font: File, size: Int, face: String, type: String, charset: String, output: String, format: String, lineHeight: Null<Int>,
		distance: Int, padding: Int
	): Void {
