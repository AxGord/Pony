...
@:nullSafety(Strict) final class Server extends NModule<ServerConfig> {
	...
	private function run(cfg: ServerConfig): Void {
		...
		if (@:nullSafety(Off) (cfg.port != null) && cfg.path != null) {
			tasks.add();
			final http: Http = new Http(cast cfg.port, cast cfg.path);
			http.onError << eError;
			http.onLog << eLog;
		}
		for (proxy in cfg.proxy) {
			tasks.add();
			final proxy: Proxy = new Proxy(proxy, cfg.port, cfg.path);
	...
	private function now(): String {
		...
		#if (haxe_ver >= 4.000)
		final z: Int = d.getTimezoneOffset();
		#else
		final z: Int = untyped __js__('new Date().getTimezoneOffset()');
		#end
