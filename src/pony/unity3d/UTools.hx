...
@:nativeGen class UTools {
	...
	public static function init(
		key: String = '', camera: String = '/Camera', defWidth: Int = 800, defHeight: Int = 600, fs: Bool = true
	): Bool {
		var args: {
			reg: Bool,
			quality: String,
			width: String,
			height: String
		} = getArgs(['quality', 'width', 'height'], key == '' ? {} : { reg: key });
		...
		compEnabled(
			cam, 'NoiseAndGrain',
			compEnabled(
				cam, 'DepthOfFieldScatter',
				compEnabled(
					cam, 'BloomAndLensFlares',
					compEnabled(
						cam, 'NoiseAndGrain', compEnabled(cam, 'DepthOfFieldScatter', compEnabled(cam, 'SSAOEffect', cfg.quality == 2))
					)
				)
			)
		);
