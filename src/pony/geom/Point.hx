...
abstract Point<T:Float>(PointImpl<T>) from PointImpl<T> to PointImpl<T> {
	...
	@:op(A / B) public static inline function div1<T:Float>(lhs: Point<T>, rhs: T): Point<Float> return { x: lhs.x / rhs, y: lhs.y / rhs };

	@:op(A / B) public static inline function div2i<T:Int>(lhs: Point<T>, rhs: Point<T>): Point<Float>
		return { x: lhs.x / rhs.x, y: lhs.y / rhs.y };

	@:op(A / B) public static inline function div1i<T:Int>(lhs: Point<T>, rhs: T): Point<Float> return { x: lhs.x / rhs, y: lhs.y / rhs };
	...
	@:op(A * B) public static inline function mul1<T:Float>(lhs: Point<T>, rhs: T): Point<T> return { x: lhs.x * rhs, y: lhs.y * rhs };

	@:op(A + B) public static inline function add2<T:Float>(lhs: Point<T>, rhs: Point<T>): Point<T>
		return { x: lhs.x + rhs.x, y: lhs.y + rhs.y };

	@:op(A + B) public static inline function add1<T:Float>(lhs: Point<T>, rhs: T): Point<T> return { x: lhs.x + rhs, y: lhs.y + rhs };
