		...
		for (f in fields) {
			...
			switch (f.kind) {
				case FVar(t, _):
					...
					fields.push({ name: 'get_${f.name}', kind: FFun({ args: [], ret: t, #if openfl
					expr: macro return untyped getChild($v{f.name}),
					#else
					expr: macro return untyped this.getChildByName($v{f.name}),
					#end params: [] }), pos: f.pos, access: [AInline, APrivate] });
					if (allowSet) // Only flash!
						fields.push({ name: 'set_${f.name}', kind: FFun(
							{ args: [{ name: 'v', type: t }], ret: t, expr: macro return untyped this[$v{f.name}] = v, params: [] }
						), pos: f.pos, access: [AInline, APrivate] });
				case _:
			}
		}
		return fields;
