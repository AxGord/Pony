...
@:nullSafety(Strict) class RubberLayoutCore<T> extends BaseLayoutCore<T> {
	...
	#if (haxe_ver >= 4.2) extern #else @:extern #end
	private inline function get_padding(): Bool return _padding;

	#if (haxe_ver >= 4.2) extern #else @:extern #end
	private inline function get_align(): Align return _align;

	#if (haxe_ver >= 4.2) extern #else @:extern #end
	private inline function get_limit(): Bool return _limit;
	...
	#if (haxe_ver >= 4.2) extern #else @:extern #end
	private inline function set_limit(v: Bool): Bool {
		if (limit == v) return v;
		_limit = v;
		update();
		return v;
