...
class PixiXmlUi extends LogableSprite implements HasAbstract {
	...
	private function createUIElement(name: String, attrs: Dynamic<String>, content: Array<Dynamic>, textContent: String): Dynamic {
		...
		final obj: DisplayObject = switch name {
			case 'free':
				...
				s;
			case 'rect':
				final color = UColor.fromString(attrs.color);
			...
			case 'mask':
				...
				o;
			case 'slice':
				final s = SliceTools.getSliceSprite(attrs.name, attrs.src, parseAndScale(attrs.creep));
				if (attrs.w != null) s.sliceWidth = parseAndScale(attrs.w);
				if (attrs.h != null) s.sliceHeight = parseAndScale(attrs.h);
				s;
			case 'clip':
				final data = if (attrs.name != null) {
					final a = attrs.name.split('|');
					final p = a[1].split('...');
					[
				...
				final m = AnimatedSprite.fromFrames(data);
			...
			case 'fastclip':
				...
				final clip = FastMovieClip.fromStorage(
			...
			case 'slider':
				...
				b;
			case 'textbutton':
				final font = parseAndScaleInt(attrs.size) + PX + attrs.font;
				final text = textTransform(putData(textContent), attrs.transform);
				new TextButton(
			...
			case 'bar':
				final b = scaleBorderInt(attrs.border);
				new Bar(
			...
			case 'vscroll':
				...
				b;
			case 'progressbar':
				final font = attrs.font == null ? null : parseAndScaleInt(attrs.size) + PX + attrs.font;
				new ProgressBar(
			...
			#if pixi_particles
			case 'particles':
				final src = attrs.src.split(',').map(StringTools.trim);
				if (attrs.img != null) {
	...
	private function createFilters(data: Dynamic<Dynamic<String>>): Void {
		...
		for (name in Reflect.fields(data)) {
			final d: Dynamic<String> = Reflect.field(data, name);
			if (d.nomobile.isTrue() && JsTools.isMobile) continue;

			final f: Filter = switch Reflect.field(d, 'extends') {
				// case 'shadow':
