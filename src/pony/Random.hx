...
@:nullSafety(Strict) class Random {
	...
	public static inline function int(to: Int, from: Int = 0): Int return from >= to ? to : from + Std.int((to - from + 1) * Math.random());

	public static inline function fromArray<T>(a: Array<T>): Null<T> return a[uint(a.length - 1)];
