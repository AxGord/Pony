...
class Builder {
	...
	macro public static function build(): Array<Field> {
		...
		for (f in fields) {
			for (m in f.meta) {
				switch m.name {
					...
					case ':activePath':
						activePathes.push({ field: f.name, expr: { expr: EObjectDecl([
						...
						switch f.kind {
							case FFun(fun):
								for (a in fun.args) switch a.type {
									case TPath(p): d.push({ expr: EObjectDecl([
					...
					case ':check':
						acc.push({ field: f.name, expr: macro $v{switch m.params[0].expr {
		...
		fields.push({ pos: Context.currentPos(), name: '__methoArgs__', meta: [], doc: null, access: [AStatic, APrivate], kind: FVar(
			null,
			{ expr: EObjectDecl(data), pos: Context.currentPos() }
		) });

		fields.push({ pos: Context.currentPos(), name: '__methoPathes__', meta: [], doc: null, access: [AStatic, APublic], kind: FVar(
			null,
			{ expr: EObjectDecl(pathes), pos: Context.currentPos() }
		) });

		fields.push({ pos: Context.currentPos(), name: '__methoActivePathes__', meta: [], doc: null, access: [AStatic, APublic], kind: FVar(
			null,
			{ expr: EObjectDecl(activePathes), pos: Context.currentPos() }
		) });

		fields.push({ pos: Context.currentPos(), name: '__methoAccess__', meta: [], doc: null, access: [AStatic, APublic], kind: FVar(
			null,
			{ expr: EObjectDecl(acc), pos: Context.currentPos() }
		) });
