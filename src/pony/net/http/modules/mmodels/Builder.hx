	...
	macro public static function build(): Array<Field> {
		...
		for (f in fields) switch (f.name) {
			case 'many', 'insert', 'single', 'update', 'delete':
				if (!f.meta.exists(function(m) return m.name == 'action')) f.meta.push({
					pos: Context.currentPos(),
					name: 'action',
					params: [{ expr: EConst(CString(f.name.bigFirst())), pos: Context.currentPos() }]
				});
			...
			for (m in f.meta) {
				switch m.name {
					...
					case ':check':
						acc.push({
							field: f.name,
							expr: macro $v{switch m.params[0].expr {
