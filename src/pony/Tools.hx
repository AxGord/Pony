...
class Tools {
	...
	public static function equal(a: Dynamic, b: Dynamic, maxDepth: Int = 1): Bool {
		...
		switch Type.typeof(a) {
			...
			#if (haxe_ver >= 4.100)
			case TObject:
				if (Std.isOfType(a, Class)) return false;
			#else
			case TObject:
				if (Std.is(a, Class)) return false;
			#end
			case TUnknown:
			case TClass(t):
				if (t == Array) {
	...
	macro public static function usedLibsDirs(): Expr {
		return macro $v{[
			for (path in getUsedLibs()) for (dir in sys.FileSystem.readDirectory(path)) dir => path
		]};
	}
	...
	macro public static function currentDir(): Expr {
		var f: String = Context.getPosInfos(Context.currentPos()).file;
		f = StringTools.replace(sys.FileSystem.fullPath(f), '\\', '/')
				.split('/')
				.slice(0, -1)
				.join('/') + '/';
		return macro $v{f};
