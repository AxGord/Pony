...
class HasAssetBuilder {
	...
	private static function addBaseFields(
		...
		fields.push({ name: 'ASSETS_LIST', access: [APrivate, AStatic], kind: FieldType.FVar(
			macro :Array<String>, macro $a{list}
		), pos: Context.currentPos() });
		fields.push({ name: 'ASSETS_NAMES', access: [APrivate, AStatic], kind: FieldType.FVar(
			macro :Array<Null<String>>, macro $a{names}
		), pos: Context.currentPos() });
		fields.push({ name: 'ASSETS_PATHES', access: [APrivate, AStatic], kind: FieldType.FVar(
			macro :Array<String>, macro $patches
		), pos: Context.currentPos() });
	}
	...
	private static function addBaseMethods(fields: Array<Field>, cl: Null<Ref<ClassType>>): Void {
		...
		fields.push({ name: 'loadAllAssets', access: [APublic, AStatic], kind: FieldType.FFun({ args: [
			{ name: 'childs', type: macro :Bool, value: macro true },
			{ name: 'cb', type: macro :Int -> Int -> Void }
		], ret: macro :Void, expr: macro childs
			? pony.ui.AssetManager.loadPackWithChilds($v{cl.toString()}, ASSETS_PATHES, ASSETS_LIST, cb)
			: pony.ui.AssetManager.loadPack(ASSETS_PATHES, ASSETS_LIST, cb) }), pos: Context.currentPos(), meta: [keepMeta] });
		fields.push({ name: 'countAllAssets', access: [APublic, AStatic], kind: FieldType.FFun({ args: [{ name: 'childs', type: macro :Bool, value: macro true }], ret: macro :Int, expr: macro return childs
			? pony.ui.AssetManager.allCountWithChilds($v{cl.toString()}, ASSETS_PATHES, ASSETS_LIST)
			: pony.ui.AssetManager.allCount(ASSETS_PATHES, ASSETS_LIST) }), pos: Context.currentPos(), meta: [keepMeta] });
	}
	...
	private static function addMethods(fields: Array<Field>, f: String, vs: String): Void {
		...
		fields.push({ name: f == 'def'
			? 'asset'
			: 'asset_' + f, access: [APublic, AStatic], kind: FieldType.FFun({ args: [{ name: 'asset', type: macro :Int }], ret: macro :String, expr: macro @:nullSafety(Off) return ASSETS_NAMES[asset] == null
			? $v + ASSETS_LIST[asset]
			: ASSETS_NAMES[asset] }), pos: Context.currentPos() });
		...
		fields.push({ name: f == 'def'
			? 'image'
			: 'image_' + f, access: [APublic, AStatic], kind: FieldType.FFun({ args: [{ name: 'asset', type: macro :Int }], ret: null, expr: macro return pony.ui.AssetManager.image(
			$v + ASSETS_LIST[asset], ASSETS_NAMES[asset]
		) }), pos: Context.currentPos() });
		fields.push({ name: f == 'def'
			? 'spine'
			: 'spine_' + f, access: [APublic, AStatic], kind: FieldType.FFun({ args: [{ name: 'asset', type: macro :Int }], ret: null, expr: macro return pony.ui.AssetManager.spine(
			$v + ASSETS_LIST[asset]
		) }), pos: Context.currentPos() });

		fields.push({ name: f == 'def'
			? 'sound'
			: 'sound_' + f, access: [APublic, AStatic], kind: FieldType.FFun({ args: [{ name: 'asset', type: macro :Int }], ret: null, expr: macro return pony.ui.AssetManager.sound(
			$v + ASSETS_LIST[asset]
		) }), pos: Context.currentPos() });

		fields.push({ name: f == 'def'
			? 'getTexture'
			: 'getTexture_' + f, access: [APublic, AStatic], kind: FieldType.FFun({ args: [{ name: 'asset', type: macro :Int }], ret: null, expr: macro return pony.ui.AssetManager.texture(
			$v + ASSETS_LIST[asset], ASSETS_NAMES[asset]
		) }), pos: Context.currentPos() });

		fields.push({ name: f == 'def'
			? 'font'
			: 'font_' + f, access: [APublic, AStatic], kind: FieldType.FFun({ args: [{ name: 'asset', type: macro :Int }], ret: null, expr: macro return pony.ui.AssetManager.font(
			$v + ASSETS_LIST[asset]
		) }), pos: Context.currentPos() });

		fields.push({ name: f == 'def'
			? 'animation'
			: 'animation_' + f, access: [APublic, AStatic], kind: FieldType.FFun({ args: [{ name: 'asset', type: macro :Int }], ret: null, expr: macro return pony.ui.AssetManager.animation(
			$v + ASSETS_LIST[asset], ASSETS_NAMES[asset]
		) }), pos: Context.currentPos() });

		fields.push({ name: f == 'def'
			? 'clip'
			: 'clip_' + f, access: [APublic, AStatic], kind: FieldType.FFun({ args: [{ name: 'asset', type: macro :Int }], ret: null, expr: macro return pony.ui.AssetManager.clip(
			$v + ASSETS_LIST[asset], ASSETS_NAMES[asset]
		) }), pos: Context.currentPos() });

		fields.push({ name: f == 'def'
			? 'text'
			: 'text_' + f, access: [APublic, AStatic], kind: FieldType.FFun({ args: [{ name: 'asset', type: macro :Int }], ret: null, expr: macro return pony.ui.AssetManager.text(
			$v + ASSETS_LIST[asset]
		) }), pos: Context.currentPos() });

		fields.push({ name: f == 'def'
			? 'bin'
			: 'bin_' + f, access: [APublic, AStatic], kind: FieldType.FFun({ args: [{ name: 'asset', type: macro :Int }], ret: null, expr: macro return pony.ui.AssetManager.bin(
			$v + ASSETS_LIST[asset]
		) }), pos: Context.currentPos() });
	}
	...
	private static function getPrefix(meta: Metadata): String {
		...
		if (meta.checkMeta([':assets_parent'])) {
			...
			switch parent.expr {
				case EConst(CIdent(s)):
					switch Context.getType(s) {
						case TInst(cl, _): parentPrefix = getPrefix(cl.get().meta.get());
						case _: Context.error('Wrong assets_parent type', parent.pos);
					}
				case EField({ expr: EConst(CIdent(pack)) }, field):
					final name: String = pack + '.' + field;
					for (f in Context.getModule(name)) switch f {
		...
		if (meta.checkMeta([':assets_prefix'])) {
			...
			switch patches.expr {
				case EConst(CString(s)): return parentPrefix + s;
				case _: Context.error('Wrong assets_prefix format', patches.pos);
			}
		}
		return parentPrefix;
	}
